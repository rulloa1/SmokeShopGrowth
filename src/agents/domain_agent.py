...
def bind_custom_domain(deploy_dir, custom_domain):
    ...
    try:
        ...
        print("  Executing Vercel CLI command...")
        ...
        if "Success" in output or "Added" in output or result.returncode == 0:
            ...
            print("  ⚠️ Action Required: Ensure your DNS records (A Record or CNAME) on your registrar point to Vercel!")
            print("     A Record: 76.76.21.21")
            print("     Alternatively, change nameservers to Vercel's nameservers.")
            return True
        else:
